...
//! the crate), complementing the white-box unit tests inside `src/`.
...
/// Happy path — many keys survive a clean reopen.
#[test]
fn hundred_keys_survive_reopen() {
    ...
    assert_eq!(db.get(b"k99"), Some(b"v99".to_vec()));
}
...
/// Corrupt CRC — a byte flipped in the last record makes it fail checksum; on
/// reopen that record is dropped (tail truncated) and earlier ones survive.
#[test]
